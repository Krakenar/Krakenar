...
public class UniqueNameAlreadyUsedException : ConflictException
{
  ...
  public UniqueNameAlreadyUsedException(User user, UserId conflictId)
    : this(user.RealmId, User.EntityType, user.EntityId, conflictId.EntityId, user.UniqueName, nameof(user.UniqueName))
  {
  }
  public UniqueNameAlreadyUsedException(RealmId? realmId, string entityType, Guid entityId, Guid conflictId, UniqueName uniqueName, string propertyName)
