...
const expectedName = computed<string>(() => props.user.fullName ?? props.user.uniqueName);
...
async function doDelete(): Promise<void> {
  if (!isDeleting.value) {
    ...
    try {
      const user: User = await deleteUser(props.user.id);
      emit("deleted", user);
      hide();
    } catch (e: unknown) {
      emit("error", e);
...
<template>
  <span>
    ...
    <TarModal :close="t('actions.close')" id="delete-user" ref="modalRef" :title="t('users.delete.title')">
      ...
      <TarInput floating id="delete-user-name" :label="t('name')" :placeholder="t('name')" required :status="status" v-model="name" />
      <template #footer>
